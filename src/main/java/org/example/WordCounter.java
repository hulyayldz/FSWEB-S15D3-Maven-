...
// hashmap kullan - hızlı search
    public static Map<String,Integer> calculatedWord() {

        String data = "When the offensive resumed, the Turks received their first victory when the Greeks encountered stiff resistance in the battles of First and Second İnönü,"
        ...
        // data regex -- noktalama işaretleri
        // data split
        // her kelime " key " olsun - tekrar sayiları value

        String regex = "[^\\w\\s]";
        String cleanData = data.replaceAll(regex, "");

        String[] newData = cleanData.split("\\s+");

        Map<String,Integer> resultData = new HashMap<>();
        List<String> wordList = Arrays.asList(newData);

        for (String dataKey: newData) {
           int tekrarSayisi = Collections.frequency(wordList, dataKey);
            dataKey = dataKey.toLowerCase();
            resultData.put(dataKey, tekrarSayisi);
        }
